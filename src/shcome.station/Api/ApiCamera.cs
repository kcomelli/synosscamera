...
using shcome.station.Internals;
...
namespace shcome.station.Api
{
    ...
    public class ApiCamera : AuthenticatedStationApiBase
    {
        ...
        /// <summary>
        /// Clear camera info cache
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        internal async Task ClearCameraInfoCache(CancellationToken cancellationToken)
        {
            var camList = await GetCameraListAsync(cancellationToken);
            if (camList.Success)
            {
                foreach (var cam in camList.Data.Cameras)
                {
                    var cacheKey = $"{CameraInfoKey}::{cam.Id}";
