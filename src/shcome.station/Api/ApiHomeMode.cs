...
namespace shcome.station.Api
{
    ...
    public class ApiHomeMode : AuthenticatedStationApiBase
    {
        ...
        public ApiHomeMode(ApiCamera cameraApi, SurveillanceStationClient client, IOptions<SurveillanceStationSettings> settings, IMemoryCacheWrapper memoryCache, ILoggerFactory loggerFactory, IStationApiUtil apiUtil)
        ...
        public async Task<StationResponseBase> SwitchHomeMode(bool on, CancellationToken cancellation = default)
        {
            if (await VerifyLoggedIn(cancellation))
            {
                ...
                {
                    Logger.LogDebug("Retrieved home mode switched response. Clearing camera info cache.");
                    await _apiCamera.ClearCameraInfoCache(cancellation);
                    Logger.LogDebug("Loading home mode info from cache or api after switching.");
